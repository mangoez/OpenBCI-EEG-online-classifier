...
def start_recording(total_sec, max_samples, params, args):
    ...
    for count, channel in enumerate(eeg_channels):
        DataFilter.perform_lowpass(data[channel], BoardShim.get_sampling_rate(args.board_id), 45, 3,
                                        FilterTypes.BUTTERWORTH.value, 0)
        DataFilter.perform_highpass(data[channel], BoardShim.get_sampling_rate(args.board_id), 1, 3,
                                        FilterTypes.BUTTERWORTH.value, 0)
...
def accept_direction(surface):
    myfont = pygame.font.SysFont('Comic Sans MS', 150)
    text = myfont.render('Left, Right or Rest?', False, (160, 160, 160))
...
def record_now():
    ...
    draw_black(win)
    direction = accept_direction(win) # Receive input from user
    ...
    print(direction)
    # save_data(single_trial_data, direction)
